...
class PixelArtPaint: #la clase para la ventana principal (Y unica ventana)
    def __init__(self, root, pixel_size=15, grid_size=30):
        ...
        self.canvas.bind("<Button-1>", self.paint) #Esta y la siguiente son la configuracion de cuando se da clic con el mouse o si se mantiene presionado
...
#La siguiente es la funcion para la accion de pintar los cuadros cuando se da clic
    def paint(self, event):
        ...
        self.canvas.create_rectangle(x, y, x + self.pixel_size, y + self.pixel_size, outline="gray", fill=color)
